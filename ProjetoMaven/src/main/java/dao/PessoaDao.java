...
public class PessoaDao {
	...
	public Number somaQtdCarros(String nome) {
		
		TypedQuery<Number> query = em.createQuery("select COUNT(p) from Pessoa p join p.veiculo on p.nome like :idpessoa", Number.class);
		query.setParameter("idpessoa", "%" + nome + "%");
	...
	public Number somaQtdCarrosNome(String nome) {
		TypedQuery<Number> query = em.createQuery("select COUNT(p) form Pessoa p join p.veiculo on p.id = :nomepessoa", Number.class);
	...
	public Number contarLetras(int id) {
		...
		TypedQuery<Number> query = em.createQuery("select LENGHT(p.nome) from Pessoa p where p.id = :idpessoa", Number.class);
